...
#   /usr/bin/make pack     # /workspace/xmp-tfmx-1.0.1.zip
#
...
pack: dll
	rm -f /workspace/xmp-tfmx-1.0.1.zip
	mkdir -p $(DIST)/pack
	cp -f $(DIST)/xmp-tfmx.dll $(ROOT)/README.md $(DIST)/pack/
	cd $(DIST)/pack && zip -9 /workspace/xmp-tfmx-1.0.1.zip xmp-tfmx.dll README.md
	rm -rf $(DIST)/pack
	ls -l /workspace/xmp-tfmx-1.0.1.zip
